...
| `sticky-notes:session` | global; the last session picked, pre-selected on every page while it is live: its Claude Code session id (survives a restart/resume), else the daemon id |
...
| `picker.js` | the session `<select>`: one live session picks itself, otherwise the last session picked anywhere (`sticky-notes:session`, matched by Claude Code session id, so it survives a Claude Code restart), otherwise "pick a session…"; `queue` is always offered and never automatic |
...
| orphans count (`N orphaned`, red, by Clear; hidden at 0; click deletes them) | `.sticky-notes-bar__orphans` |
| bar (fixed, left of the pin) | `.sticky-notes-bar`, buttons `.sticky-notes-bar__button` (`[aria-pressed=true]` while picking, `--busy` with the "capturing 2/5" / "sending…" label while Send runs), `.sticky-notes-bar__count` (inside Add note), `.sticky-notes-bar__tool` (Add note / Screenshot / Clear tiles), `.sticky-notes-bar__group` (`--deliver` = picker + `--send`; a successful Send clears the notes unless `clearOnSend: false`), `.sticky-notes-bar__more` (`<details>` ⋯ menu: `__menu`, `__item`; holds the exports, Download and the `__auto` auto-shot checkbox row with `__check`), `.sticky-notes-bar__picker` (session `<select>`), `.sticky-notes-bar__shots` (unsent shot count) |
| export pane | `.sticky-notes-export` |
| leaders svg | `.sticky-notes-leaders` |
| note | `.sticky-note`, `.sticky-note--dragging`, `.sticky-note__header`, `.sticky-note__index`, `.sticky-note__path` (the quoted element text; the CSS path in its `title`), `.sticky-note__button` (`data-command="collapse"` / `"remove"`), `.sticky-note__text` (textarea) |
