...
| session picker | live review sessions by Claude Code name (`/rename` › auto title › derived `dev-2f`), the folder added only when sessions differ by it; this app's own first; a single live session picks itself, `queue` never does. The last pick is remembered for the whole browser and pre-selected on every page while that session is live |
| auto-shot (⋯ menu, on by default) | on Send, every noted element without a manual shot is captured as a JPEG (1568 px cap) so you see what the note points at |
